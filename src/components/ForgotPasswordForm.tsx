...
export const ForgotPasswordForm = () => {
  ...
  const handleForgotPassword: SubmitHandler<ForgotPasswordInput> = async (data: ForgotPasswordInput) => {
    try {
      ...
      <Grid item xs={12} sm={8} md={5} component={Paper} elevation={6} square sx={{ borderRadius: "20px", padding: "1em" }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <Button
            fullWidth
            variant="contained"
            sx={{ mt: 2, mb: 2, backgroundColor: "rgba(45, 146, 42, 0.37)", "&:hover": { backgroundColor: "green" } }}
            onClick={() => navigator("/")}
          >
            Voltar
          </Button>
